...
Meteor.connection._processOneDataMessage = function (msg, updates) {
  ...
  const activeRegSub = Object.values(Meteor.connection._subscriptions).some(s => s.name.includes(collectionName));

  // keep track of added ids when using publish.once so that we can clean them up with removeFromMiniMongo
  if (messageType === 'added') {
    const activeSub = Object.values(subsCache).find(s => s.active && s.handle._collectionNames?.includes(collectionName));
    if (!activeRegSub && activeSub) {
      msg.fields.__sub = activeSub.handle.subscriptionId
    }
  }

  const ignore = (messageType === 'added' && serverDoc?.document?._id)
    || (messageType === 'removed' && existingDoc && existingDoc.__sub)
    || (!existingDoc && (messageType === 'changed' || messageType === 'removed' && !serverDoc?.document));

  if (activeRegSub) { // sub may have been added if you have a regular publication and a .once publication on the same collection and you're navigating between them. if that's the case, we want to remove the sub when the subscription from regular publication becomes the active one.
