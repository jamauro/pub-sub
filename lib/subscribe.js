...
import { extractSubscribeArguments, mergeIntoMinimongo, removeFromMinimongo } from './utils-client';
import { PubSub } from './config';

const ignoredSubs = ['meteor.loginServiceConfiguration', 'meteor_autoupdate_clientVersions', 'tinytest_results_subscription'];
...
function subscribe(name, ...args) {
  ...
  // if we are inside a Tracker computation, we stop when it stops -- e.g. when a template is destroyed
  if (c?.firstRun) {
  ...
  if (cachedSubHandle) {
    ...
    if (remaining > 0) { // if the sub was cached and has time remaining, just reuse it
      ...
      if (cachedDuration - remaining < 10) { // subscribe can fire multiple times very quickly if you aren't careful, most of the time it's harmless but in this case we want to return early
        return cachedSubHandle;
      }

      if (cachedIds) {
        Promise.resolve().then(() => { // used to make this run after the .stop is called on the previous handle
          const selector = {_id: {$in: cachedIds}};
          const modifier = isSimulated ? {$set: {__sub: cachedSubHandle.subscriptionId}} : {$unset: {__sub: ''}};

          for (const collectionName of cachedSubHandle._collectionNames) { // using for loop for the performance
            const localCollection = Meteor.connection._mongo_livedata_collections[collectionName];
            localCollection.update(selector, modifier, {multi: true});
          }

          // clean up cachedIds. this isn't absolutely necessary but it's an optimization that cleans up data that we shouldn't need anymore. if it causes issues, it can be safely removed. the timeout on the sub will eventually remove the all the data for that sub.
  ...
  if (!Meteor.settings.public.packages?.['jam:pub-sub']?.pubs?.includes(name)) {
    const nameLowerCased = name.toLowerCase();
    const handle = originalSubscribe.call(this, name, ...args);
    handle._collectionNames = Object.keys(Meteor.connection._mongo_livedata_collections).filter(c => nameLowerCased.includes(c));

    if (shouldCache && c?.firstRun) {
      let stopped = false;
      const originalStop = handle.stop;
      handle.stop = () => {
        if (stopped) return; // prevents .stop from being called twice, not sure why this happens. might be a Meteor bug.
        stopped = true;
        ...
        const ids = handle._collectionNames.flatMap(collectionName => {
          const localCollection = Meteor.connection._mongo_livedata_collections[collectionName];
          return localCollection.find({ __sub: {$exists: false} }).map(d => d._id);
        });
  ...
  const subHandle = {
    ...
    stop() {
      ...
      const ids = _collectionNames.flatMap(collectionName => {
        const localCollection = Meteor.connection._mongo_livedata_collections[collectionName];
        return localCollection.find({ __sub: subscriptionId }).map(d => d._id);
      });
