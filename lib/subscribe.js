...
import { subs, extractSubscribeArguments, mergeIntoMinimongo, removeFromMinimongo } from './utils/client';
...
function subscribe(name, ...args) {
  ...
  const { handle: cachedSubHandle, duration: cachedDuration, timeoutId } = (shouldCache && findSub(key)) || {};
  ...
  const subHandle = {
    ...
    stop() {
      ...
      if (!shouldCache) {
        ...
        return Promise.resolve().then(() => removeFromMinimongo(_collectionNames, subscriptionId));
      }
