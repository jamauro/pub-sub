...
function subscribe(name, ...args) {
  ...
  if (!isOnce) {
    ...
    handle.stop = (invalidated) => {
      if (c && !invalidated) return; // some view layers like react with react-meteor-data weirdly call .stop a bunch, we only want this to run when the component containing the subscription has been destroyed. other tracker integrations, like vue-meteor-tracker, don't have a Tracker.currentComputation so we check first for its existence.
