...
export const convertObjectId = o => {
  if (Array.isArray(o)) {
    ...
    for (const key in o) {
      ...
      if (key === '_str') { // convert from Meteor ObjectId to Mongo ObjectId
        return new ObjectId(v)
      }

      if (v instanceof Mongo.ObjectID) { // convert from Meteor ObjectId to Mongo ObjectId
        newSelector[key] = new ObjectId(v.valueOf());
      } else if (v instanceof ObjectId) { // convert from Mongo ObjectId to Meteor ObjectId
        newSelector[key] = new MongoID.ObjectID(v.toHexString());
      } else if (typeof v === 'object' && v !== null) {
        newSelector[key] = convertObjectId(v)
      } else {
        newSelector[key] = v;
      }
    }
