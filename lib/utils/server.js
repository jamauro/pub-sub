...
export const createProjection = modifier => {
  ...
  let softDeletedField = null
  if (Package['jam:soft-delete']) {
    softDeletedField = SoftDelete.config.deleted;
  }
  ...
  for (const key in modifier) {
    ...
    for (const [field, value] of Object.entries(modifier[key])) {
      if (key === '$unset') unsets.push(field);
      projection[field.split('.')[0]] = 1; // top level field only
      if (softDeletedField && field.split('.')[0] === softDeletedField && value === true) {
        softDeleted = true;
      }
    }
  }

  return { projection, unsets, softDeleted };
};
